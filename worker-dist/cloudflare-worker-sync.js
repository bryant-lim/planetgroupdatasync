...
async function runSync(env) {
  ...
  if (convIds.length > 0) {
    try {
      ...
      const { data: existingRows } = await supabase.from("conversations").select("id, customer_name, conversation_summary, conversation_tags, conversation_transcript, current_salary, job_title").or(orQuery);
      if (existingRows) {
