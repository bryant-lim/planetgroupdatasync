...
1. Install Wrangler globally or run it via npx:
   ...
   ```
2. Deploy the Worker script [`cloudflare-worker-sync.ts`](file:///Users/bryantlim/Documents/chillor-repo/nxlink-sync-planetgroup/cloudflare-worker-sync.ts) using the configuration in [`wrangler.toml`](file:///Users/bryantlim/Documents/chillor-repo/nxlink-sync-planetgroup/wrangler.toml):
   ```bash
   npx wrangler deploy
   ```
3. Copy the URL of your deployed Worker (e.g., `https://nxlink-sync-planetgroup-worker.yourdomain.workers.dev/`). You will need this URL for the Pages setup.
